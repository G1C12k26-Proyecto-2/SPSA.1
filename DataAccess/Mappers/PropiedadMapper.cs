...
namespace DataAccess.Mappers
{
    public class PropiedadMapper : IObjectMapper, ICrudStatements
    {
        ...
        public BaseClass BuildObject(Dictionary<string, object> row)
        {
            ...
            propiedad.Active = bool.Parse(row["Active"].ToString());
            ...
            if (row.ContainsKey("ValorCalculado") && row["ValorCalculado"] != null)
            {
                propiedad.ValorCalculado = decimal.Parse(row["ValorCalculado"].ToString());
            }

            if (row.ContainsKey("FechaUltimaValuacion") && row["FechaUltimaValuacion"] != null && row["FechaUltimaValuacion"] != DBNull.Value)
            {
